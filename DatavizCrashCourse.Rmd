...
seq_questionnaire <- as_tibble(read.csv(file.path("./","Questionnaire for Experiment L(1-10).csv"), sep=";", fileEncoding="UTF-8-BOM"))
non_seq_questionnaire <- as_tibble(read.csv(file.path("./","Questionnaire for Experiment UL(1-10).csv"), sep=";", fileEncoding="UTF-8-BOM"))

seq_questionnaire$condition <- "Sequential"
non_seq_questionnaire$condition <- "non-Sequential"


df_merge <- rbind(seq_questionnaire,non_seq_questionnaire)
```
...
ggplot(data = df_merge, aes(x=condition, y=I.enjoyed.doing.this.activity.very.much2, color=condition)) + 
  theme_bw() + 
  geom_violin() + 
  geom_boxplot(width=0.1) +
  geom_jitter(alpha=0.1)+ ylim(1, 7)
...
ggplot(data = df_merge, aes(x=I.enjoyed.doing.this.activity.very.much, y=I.enjoyed.doing.this.activity.very.much2, color=condition)) + 
  geom_jitter() + 
  theme_bw() + 
  geom_smooth(method = "lm",se=F)
```
...
shapiro.test(df_merge$I.enjoyed.doing.this.activity.very.much2)
...
t.test(data=df_merge, I.enjoyed.doing.this.activity.very.much2~condition)

#A wilcox test works the same exact way
wilcox.test(data=df_merge, I.enjoyed.doing.this.activity.very.much~condition)

wilcox.test(data=df_merge, I.enjoyed.doing.this.activity.very.much2~condition)


wilcox.test(questionnaire$AgencyNew, questionnaire$AgencyOld)

cor.test(df_merge$I.enjoyed.doing.this.activity.very.much, df_merge$I.enjoyed.doing.this.activity.very.much2)
```
